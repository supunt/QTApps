...
QString logger::_default_log_path = QDir::tempPath() +"\\Peercore\\FTPClient";
...
//-----------------------------------------------------------------------------------------------------------------------------------------
 void logger::logThis(QString data, QString typePrefix)
 {
     ...
     {
             _syncMan->report("Log stream broken. Contact peercore",LOGGER,ERROR);
     }
 }
//-----------------------------------------------------------------------------------------------------------------------------------------
 bool logger::init(QString &error, syncManager* syncMan)
 {
    ...
    if (QFile::exists(logFile))
    {
        if (!renameLog())
        {
            error = "Failed to rename existing log '" +_logPath + "'. Contact Peercore Nexgen";
            return false;
        }
    }
...
bool logger::createDefaultLogPath(QString &err)
{
    ...
    {
         qDebug() << QDir::tempPath() +  "/peercore/ exisits";
    }
    return true;
}
